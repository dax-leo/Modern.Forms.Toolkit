...
namespace GMap.NET.WindowsForms
{
    ...
    public class GMapPolygon : MapRoute, ISerializable, IDeserializationCallback, IDisposable
    {
        ...
        internal bool IsInsideLocal(int x, int y)
        {
            ...
            {
                return _graphicsPath.Contains(x, y); // _graphicsPath.IsVisible(x, y); 
            }
        ...
        internal void UpdateGraphicsPath()
        {
            ...
            {
                var pnts = new SKPoint[LocalPoints.Count];
                for (int i = 0; i < LocalPoints.Count; i++)
                {
                    var p2 = new SKPoint((int)LocalPoints[i].X, (int)LocalPoints[i].Y);
                    pnts[pnts.Length - 1 - i] = p2;
                }

                if (pnts.Length > 2)
                {
                    _graphicsPath.AddPoly(pnts);
                }
        ...
        public virtual void OnRender(SKCanvas g)
        {
            if (IsVisible)
            {
                if (IsVisible)
                {
                    if (_graphicsPath != null)
                    {                        
                        g.DrawPath(_graphicsPath, new SKPaint() { Color = SKColors.Red, StrokeWidth = 3 });
                    }
                }
            }
        }
