...
namespace GMap.NET.WindowsForms
{
    ...
    public class GMapRoute : MapRoute, ISerializable, IDeserializationCallback, IDisposable
    {
        ...
        internal bool IsInside(int x, int y)
        {
            ...
            {
                return _graphicsPath.Contains(x, y);//.IsOutlineVisible(x, y, Stroke);
            }

            return false;
        }

        SKPath _graphicsPath;

        internal void UpdateGraphicsPath()
        {
            if (_graphicsPath == null)
            {
                _graphicsPath = new SKPath();
            }
            else
            {
                _graphicsPath.Reset();
...

            {
                for (int i = 0; i < LocalPoints.Count; i++)
                {
                    ...
                    if (i == 0)
                    {
                        _graphicsPath.AddPoly(new SKPoint[] { new SKPoint(p2.X, p2.Y), new SKPoint(p2.X, p2.Y) });
                    }
                    else
                    {
                        var p = _graphicsPath.LastPoint;//.GetLastPoint();
                        //_graphicsPath.AddLine(p.X, p.Y, p2.X, p2.Y);
                        _graphicsPath.AddPoly(new SKPoint[] { new SKPoint(p.X, p.Y), new SKPoint(p2.X, p2.Y) });
                    }
                }
            }
        }
        ...
        public virtual void OnRender(SKCanvas g)
        {
            if (IsVisible)
            {
                if (_graphicsPath != null)
                {
                    ...
                    g.DrawPath(_graphicsPath, new SKPaint());
                }
            }
        }

        public static readonly SKPaint DefaultStroke = new SKPaint()
        {
