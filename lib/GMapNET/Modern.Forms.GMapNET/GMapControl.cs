...
namespace Modern.Forms.GMapNET
{
    /// <summary>
    ///     GMap.NET control for Windows Forms
    /// </summary>
    public partial class GMapControl : Control, Interface
    {
        /// <summary>
        ...
        public SKPaint ScalePen = new SKPaint() { Color = SKColors.Black, StrokeWidth = 3 };
        public SKPaint ScalePenBorder = new SKPaint() { Color = SKColors.WhiteSmoke, StrokeWidth = 6 };        
        public SKPaint CenterPen = new SKPaint() { Color = SKColors.Red, StrokeWidth = 1 };
        private SKFont _copyRightFont = new SKFont(SKTypeface.Default, size: 6);
        /// <summary>
        ///     area selection pen
        /// </summary>        
        public SKPaint SelectionPen = new SKPaint() { Color = SKColors.Blue, StrokeWidth = 2 };

        SKColor _selectedAreaFill = new SKColor(Color.FromArgb(33, Color.RoyalBlue).R, Color.FromArgb(33, Color.RoyalBlue).B, Color.FromArgb(33, Color.RoyalBlue).G, Color.FromArgb(33, Color.RoyalBlue).A);
        SKColor _selectedAreaFillColor = new SKColor(SKColors.RoyalBlue.Red, SKColors.RoyalBlue.Green, SKColors.RoyalBlue.Blue, 33); //.FromArgb(33, Color.RoyalBlue);
        ...
        public SKColor SelectedAreaFillColor
        {
            get
            {
            ...
            set
            {
                if (_selectedAreaFillColor != value)
                {
                    ...
                    _selectedAreaFill = new SKColor(_selectedAreaFillColor.Red, _selectedAreaFillColor.Green, _selectedAreaFillColor.Blue, _selectedAreaFillColor.Alpha);
                }
            }
        }
        ...
        SKColor _emptyTileBrush = SKColors.Navy;
        SKColor _emptyTileColor = SKColors.Navy;
        ...
        public SKColor EmptyTileColor
        {
            ...
            set
            {
                if (_emptyTileColor != value)
                {
                    ...
                    _emptyTileBrush = new SKColor(_emptyTileColor.Red, _emptyTileColor.Green, _emptyTileColor.Blue, _emptyTileColor.Alpha);
                }
            }
        }
        ...
        public GMapControl()
        {
            ...
            {               
                ...
                OnTileLoadComplete += (elapsedMilliseconds) => { this.Invalidate(); };
        ...
        public Color EmptyMapBackground = Color.Red;// WhiteSmoke;
        ...
        void DrawGraphics(SKCanvas g)
        {
            ...
            if (_mapRenderTransform.HasValue)
            {
                ...
                g.Scale(_mapRenderTransform.Value);
            ...
            else
            {
                ...
                else
                {
                    if (!MobileMode)
                    {
                        g.Translate(Core.RenderOffset.X, Core.RenderOffset.Y);
                    }
                            ...
                            foreach (GMapImage img in t.Overlays)
                            {
                                if (img != null && img.Img != null)
                                {
                                    ...
                                    if (!img.IsParent)
                                    {
                                        ...
                                        {                                            
                                            g.DrawImage(img.Img, new SKRect(Core.TileRect.X,
                                                Core.TileRect.Y,
                                                Core.TileRect.Right,
                                                Core.TileRect.Bottom));
                                        }
                                        else
                                        {                                            
                                            g.DrawImage(img.Img, new SKRect((int)Core.TileRect.X,
                                                    (int)Core.TileRect.Y,
                                                    (int)Core.TileRect.Right,
                                                    (int)Core.TileRect.Bottom));
                                        }
                                    }
                                    else
                                    {
                                        ...
                                        g.DrawImage(img.Img,
                                            new SKRect(srcRect.X, srcRect.Y, srcRect.Width, srcRect.Height),
                                            new SKRect(dst.X, dst.Y, dst.Right, dst.Bottom));
                                    }
                                }
                            }
                        }
                        else if (FillEmptyTiles && MapProvider.Projection is MercatorProjection)
                        {
                                ...
                                // render tile 
                                {
                                    foreach (GMapImage img in parentTile.Overlays)
                                    {
                                        if (img != null && img.Img != null && !img.IsParent)
                                        {
                                            ...
                                            g.DrawImage(img.Img,
                                               new SKRect(srcRect.X, srcRect.Y, srcRect.Right, srcRect.Bottom),
                                               new SKRect(dst.X, dst.Y, dst.Right, dst.Bottom));

                                            //g.FillRectangle(dst, _selectedAreaFill);
                                        }
                                    }
                                }
                            }
                            ...
                            if (ShowTileGridLines)
                            {
                                g.DrawRectangle(
                                    ...
                                    (int)Core.TileRect.Y,
                                    (int)Core.TileRect.Width,
                                    (int)Core.TileRect.Height, SKColors.Red);

                                g.DrawText((tilePoint.PosXY == Core.CenterTileXYLocation ? "CENTER: " : "TILE: ") + tilePoint.ToString(),
                                    (float)Core.TileRect.X, (float)Core.TileRect.Y, new SKPaint() { Color = SKColors.Red });
                            }
                        }
                    }
                }
        ...
        protected virtual void OnPaintOverlays(SKCanvas g)
        {
            ...
            {                
                ...
                if (!SelectedArea.IsEmpty)
                {
                    ...
                    g.DrawRectangle((int)x1, (int)y1, (int)(x2 - x1), (int)(y2 - y1), SelectionPen.Color);
                    g.FillRectangle((int)x1, (int)y1, (int)(x2 - x1), (int)(y2 - y1), _selectedAreaFill);
                }

                // ! implement MousePosition field
                //if (_renderHelperLine)
                //{
                //    var p = FindForm()!.PointToClient(MousePosition);
                ...
                if (ShowCenter)
                {
                    g.DrawLine(Width / 2 - 5, Height / 2, Width / 2 + 5, Height / 2, CenterPen);
                    g.DrawLine(Width / 2, Height / 2 - 5, Width / 2, Height / 2 + 5, CenterPen);
                    //g.DrawCircle(Width / 2, Height / 2, 20, new SKPaint() { Color = SKColors.Red });
                }
                ...
                {                    
                    g.DrawText(Core.Provider.Copyright, 3f, Height - Theme.FontSize - 5f, _copyRightFont, new SKPaint() { Color = SKColors.Navy });
                }
        ...
        private void DrawScale(SKCanvas g, int top, int right, int bottom, int left, string caption)
        {
            g.DrawLine(left, top, left, bottom, ScalePenBorder);
            g.DrawLine(left, bottom, right, bottom, ScalePenBorder);
            g.DrawLine(right, bottom, right, top, ScalePenBorder);

            g.DrawLine(left, top, left, bottom, ScalePen);
            g.DrawLine(left, bottom, right, bottom, ScalePen);
            g.DrawLine(right, bottom, right, top, ScalePen);

            //g.DrawText(caption, ScaleFont, Brushes.Black, right + 3, top - 5);
            g.DrawText(caption, (float)right + 3, (float)top - 5, new SKPaint() { Color = SKColors.Black });
        }
        ...
        void UpdateBackBuffer()
        {
            ...
            _backBuffer = new SKBitmap(ScaledWidth, ScaledHeight);
            //_gxOff = SKCanvas.FromImage(_backBuffer);
        }
        ...
        /// </summary>
        public Size DragSize = new Size(5, 5);// SystemInformation.DragSize;
...
#pragma warning restore SYSLIB0011 // Type or member is obsolete
        ...
        public void DeserializeOverlays(Stream stream)
        {
            ...
            var overlayArray = JsonSerializer.Deserialize<GMapOverlay[]>(stream);
