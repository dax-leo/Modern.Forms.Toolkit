            ...
            if (!_wordsSizeMeasured)
            {
                ...
                if (Words.Count > 0)
                {
                    foreach (var boxWord in Words)
                    {
                        ...
                        boxWord.Height = ActualFont.Size + 4 *ModernFormsAdapter.Instance.ScalingFactor;                                               
                    }
                }
        ...
        protected void PaintBackground(RGraphics g, RRect rect, bool isFirst, bool isLast)
        {
            if (rect.Width > 0 && rect.Height > 0)
            {
                ...
                if (brush != null)
                {
                    ...
                    {
                        g.DrawRectangle(brush, (float)Math.Ceiling(rect.X), (float)Math.Ceiling(rect.Y), (float)rect.Width, (float)rect.Height);
                    }
                    ...
                    if (!word.IsLineBreak)
                    {
                        ...
                        if (clip != RRect.Empty)
                        {
                            ...
                            if (word.Selected)
                            {
                                ...
                                g.DrawRectangle(GetSelectionBackBrush(g, false), (float)rect.X, (float)rect.Y, (float)rect.Width, (float)rect.Height);
                                ...
                                if (HtmlContainer.SelectionForeColor != RColor.Empty && (word.SelectedStartOffset > 0 || word.SelectedEndIndexOffset > -1))
                                {
                                    ...
                                    g.DrawString(word.Text, ActualFont, ActualColor, wordPoint, isRtl);
                                    g.PopClip();
                                    g.PushClip(rect);
                                    g.DrawString(word.Text, ActualFont, GetSelectionForeBrush(), wordPoint, isRtl);
                                    g.PopClip();
                                }
                                else
                                {
