    ...
    internal sealed class CssBoxFrame : CssBox
    {
        ...
        private void DrawImage(RGraphics g, RPoint offset, RRect rect)
        {
            if (_imageWord.Image != null)
            {
                if (rect.Width > 0 && rect.Height > 0)
                {
                    ...
                    {
                        g.DrawRectangle(
                            GetSelectionBackBrush(g, true),
                            (float)(_imageWord.Left + offset.X), 
                            (float)(_imageWord.Top + offset.Y), 
                            (float)(_imageWord.Width + 2),
                            (float)DomUtils.GetCssLineBoxByWord(_imageWord).LineHeight);
                    }
                }
            }
            else if (_isVideo && !_imageLoadingComplete)
            {
                ...
                {
                    g.DrawRectangle(g.GetPen(RColor.LightGray), (float)rect.X, (float)rect.Y, (float)rect.Width, (float)rect.Height);
                }
            }
        }
        ...
        private void DrawTitle(RGraphics g, RRect rect)
        {
            if (_videoTitle != null && _imageWord.Width > 40 && _imageWord.Height > 40)
            {
                ...
                g.DrawRectangle(g.GetSolidBrush(RColor.FromArgb(160, 0, 0, 0)), (float)rect.Left, (float)rect.Top, (float)rect.Width, (float)(ActualFont.Height + 7));

                var titleRect = new RRect(rect.Left + 3, rect.Top + 3, rect.Width - 6, rect.Height - 6);
                g.DrawString(_videoTitle, font, RColor.WhiteSmoke, titleRect.Location, false);
            }
        }
        ...
        private void DrawPlay(RGraphics g, RRect rect)
        {
            if (_isVideo && _imageWord.Width > 70 && _imageWord.Height > 50)
            {
                ...
                g.DrawRectangle(g.GetSolidBrush(RColor.FromArgb(160, 0, 0, 0)), (float)left, (float)top, (float)size.Width, (float)size.Height);
