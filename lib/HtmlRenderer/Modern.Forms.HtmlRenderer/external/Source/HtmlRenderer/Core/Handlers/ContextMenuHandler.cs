        ...
        public void ShowContextMenu(RControl parent, CssRect rect, CssBox link)
        {
            try
            {
                ...
                if (_contextMenu.ItemsCount > 0)
                {
                    ...
                    var form_loc = ((ControlAdapter)parent)
                        .Control.PointToScreen(
                        new Point((int)parent.MouseLocation.X, (int)parent.MouseLocation.Y));
                    
                    _contextMenu.Show(parent, new Adapters.Entities.RPoint(form_loc.X, form_loc.Y));
                }
            }
            catch (Exception ex)
            {
