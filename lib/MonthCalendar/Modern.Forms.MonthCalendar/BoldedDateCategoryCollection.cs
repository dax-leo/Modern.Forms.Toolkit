namespace Modern.Forms.MonthCalendar
{
    ...
    public class BoldedDateCategoryCollection : List<BoldedDateCategory>
    {
        ...
        public new bool Remove(BoldedDateCategory item)
        {
            if (base.Remove(item))
            {
                parent.BoldedDatesCollection.RemoveAll(d => d.Category.Equals(item));
        ...
        public new int RemoveAll(Predicate<BoldedDateCategory> match)
        {
            ...
            var matches = FindAll(match);

            if (matches.Count != 0)
            {
                matches.ForEach(t => Remove(t));
            }
        ...
        {
            parent.BoldedDatesCollection.Clear();
            base.Clear();
        }
        ...
        {
            return !category.IsEmpty && !Exists(t => string.Compare(category.Name, t.Name, StringComparison.OrdinalIgnoreCase) == 0);
        }
    }
}