    ...
    internal class MonthCalendarDate : ICloneable
    {
        ...
        public DateTime Date
        {
            get
            {
                ...
                {
                    date = calendar.ToDateTime(Year, Month, Day, 0, 0, 0, 0, Era);
                }
        ...
        public MonthCalendarDate FirstOfMonth
        {
            get
            {
                ...
                {
                    if (Date == calendar.MinSupportedDateTime.Date)
                    {
                        firstOfMonth = Clone() as MonthCalendarDate;
                    }
                    else
                    {
                        firstOfMonth = new MonthCalendarDate(calendar, calendar.ToDateTime(Year, Month, 1, 0, 0, 0, 0, Era));
                    }
                }
        ...
        public MonthCalendarDate GetFirstDayInWeek(ICustomFormatProvider provider)
        {
            ...
            MonthCalendarDate dt = (MonthCalendarDate)Clone();

            while (dt.DayOfWeek != firstDayOfWeek && dt.Date > calendar.MinSupportedDateTime)
            {
                dt = dt.AddDays(-1);
            }
        ...
        {
            return new MonthCalendarDate(calendar, InternalAddMonths(months));
        }
        ...
        public MonthCalendarDate AddDays(int days)
        {
            ...
            while ((dt > calendar.MinSupportedDateTime && sign == -1 || dt < calendar.MaxSupportedDateTime.Date && sign == 1) && days != daysAdded)
            {
                dt = calendar.AddDays(dt, sign);
                daysAdded++;
            }

            return new MonthCalendarDate(calendar, dt);
        }
        ...
        public string ToString(string format, IFormatProvider fp, ICustomFormatProvider nameProvider = null, string[] nativeDigits = null)
        {
            if (fp != null)
            {
                ...
                if (dtfi != null)
                {
                    return ToString(format, dtfi, nameProvider, nativeDigits);
                }
            }

            return ToString(format, null, nameProvider, nativeDigits);
        }
        ...
        private DateTime InternalAddMonths(int months)
        {
            ...
            return months < 0 ? calendar.MinSupportedDateTime.Date : calendar.MaxSupportedDateTime.Date;
        }
                    ...
                    case 'd':
                        {
                            ...
                            sb.Append(tokLen <= 2
                               ? DateMethods.GetNumberString(Day, nativeDigits, tokLen == 2)
                               : GetDayName(c.GetDayOfWeek(Date), dtfi, nameProvider, tokLen == 3));
                    ...
                    case 'M':
                        {
                            ...
                            sb.Append(tokLen <= 2
                               ? DateMethods.GetNumberString(Month, nativeDigits, tokLen == 2)
                               : GetMonthName(Month, Year, dtfi, nameProvider, tokLen == 3));
                    ...
                    case 'y':
                        {
                            ...
                            sb.Append(tokLen <= 2
                               ? DateMethods.GetNumberString(Year % 100, nativeDigits, true)
                               : DateMethods.GetNumberString(Year, nativeDigits, false));
                    ...
                    case 'g':
                        {
                            ...
                            sb.Append(nameProvider != null
                               ? nameProvider.GetEraName(c.GetEra(Date))
                               : dtfi.GetEraName(c.GetEra(Date)));
