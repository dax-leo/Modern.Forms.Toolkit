...
namespace Modern.Forms
{
    ...
    public partial class Control : Component, ILayoutable, IArrangedElement, IDisposable
    {
        ...
        public void Invalidate(Rectangle rectangle)
        {
            ...
            DirtyByInteresction(this);

            SetAllChildrenDirty(this);// propagate dirty state to all childs
        ...
        public void Invalidate(Rectangle rectangle, bool ValidateCurrentOnly)
        {
            ...
            SetAllChildrenDirty(this); // Propagate dirty state to all child controls.
        ...
        /// <summary>
        /// Invalidate any control that intersects current control or any other intersected one.       
        /// </summary>
        private void DirtyByInteresction(Control owner)
        {
            ...
            foreach (var c in Parent.Controls.GetAllControls(true))
            {
                ...
                {
                    //Debug.WriteLine (owner.GetType ().Name + " - intersects -> " + c.GetType ().Name);

                    // recur. check for intersections
                    c.Invalidate();
                    DirtyByInteresction(c);
                }
            }
        }
        ...
        /// </summary>
        /// <param name="parent">Parent control to invalidate</param>
        ...
        public int LogicalToDeviceUnits(int value)
        {
            if (SkiaExtensions.IgnorePixelScaling)
                return DpiHelper.LogicalToDeviceUnits(value, 96);// (int)((DeviceDpi - 96) * SkiaExtensions.ScalingReduction + 96));//96
        ...
        //{
        //    if (SkiaExtensions.IgnorePixelScaling)
        //        return DpiHelper.LogicalToDeviceUnits (value, (int)((DeviceDpi - 96) * SkiaExtensions.ScalingReduction + 96));//96
        //    return DpiHelper.LogicalToDeviceUnits (value, DeviceDpi);
        //}
        ...
        public Point PointToScreen(Point point)
        {
            ...
            if (this is ControlAdapter)
            {
                ...
                // Offset adjustment for case [IgnorePixelScaling = true]
                if (SkiaExtensions.IgnorePixelScaling)
                {
                    var dx = ((int)(point.X * window.Scaling) - point.X);
                    var dy = ((int)(point.Y * window.Scaling) - point.Y);
                    point.Offset(dx, dy);
                }
        ...
        internal void RaiseMouseMove(MouseEventArgs e)
        {
            // If something has the mouse captured, they get all the events
            var captured = Controls.GetAllControls().LastOrDefault(c => c.Capture);
            //Debug.WriteLine("MOVED: " + DateTime.Now + "   "  + captured?.GetType());
            if (captured != null)
            {
        ...
        //public bool CaputureMouse = true;
        /// <summary>
