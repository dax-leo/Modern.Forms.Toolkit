...
namespace Modern.Forms
{
    ...
    public abstract class WindowBase : Component
    {
        ...
        private void OnInput (RawInputEventArgs e)
        {
            if (e is RawPointerEventArgs me) {
                ...
                if (!SkiaExtensions.IgnorePixelScaling)
                    me.Position *= window.RenderScaling; // for normal scaling correction (apply this even if IgnorePixelScaling is not used)
                else {
                    // If cursor close to borders then apply (me.Position *= window.RenderScaling) position correction due to scaling
                    // otherwise ignore it, because canvas is scaled and mouse pointer is always above unscaled location.
                    if ((int)(me.Position.X) >= this.Bounds.Width - 2
                        && (int)(me.Position.X) <= this.Bounds.Width + 2)
                        me.Position *= window.RenderScaling;

                    if ((int)(me.Position.Y) >= this.Bounds.Height - 2
                        && (int)(me.Position.Y) <= this.Bounds.Height + 2)
                        me.Position *= window.RenderScaling;
                }
        ...
        //IFramebufferPlatformSurface skia_framebuffer;// = window.Surfaces.OfType<IFramebufferPlatformSurface>().First();
        //SKSurface surface;
        System.Drawing.Size scaled_client_size;
        System.Drawing.Rectangle scaled_display_rect;
        ...
        //        framebuffer.Format.ToSkColorType (), framebuffer.Format == WindowKit.Platform.PixelFormat.Rgb565 ? SKAlphaType.Opaque : SKAlphaType.Premul);

        //    using var surface = SKSurface.Create (framebufferImageInfo, framebuffer.Address, framebuffer.RowBytes);

        //    // adapt window scaling according to scaling mode level
        //    var mod_scaling = SkiaExtensions.IgnorePixelScaling
        //        ? Math.Round((Scaling - 1) * SkiaExtensions.ScalingReduction + 1, 1)
        //        : Scaling;

        //    //var userVisibleSize = SkiaExtensions.IgnorePixelScaling ? new System.Drawing.Size ((int)(Size.Width * mod_scaling), (int)(Size.Height * mod_scaling)) : ScaledSize;
        //    var userVisibleSize = SkiaExtensions.IgnorePixelScaling ? this.Size : ScaledSize;
        //    var CanvasSize = new SKSizeI (userVisibleSize.Width, userVisibleSize.Height);

        //    scaled_client_size = new System.Drawing.Size (
        //        (int)(window.ClientSize.Width * mod_scaling),
        //        (int)(window.ClientSize.Height * mod_scaling)); //ScaledClientSize;

        //    scaled_display_rect = ScaledDisplayRectangle;

        //    if (SkiaExtensions.IgnorePixelScaling) {
        //        var canvas = surface.Canvas;
        //        canvas.Scale ((float)(Scaling / mod_scaling));
        //        canvas.Save ();
        //        scaled_client_size = this.Size;
        //        scaled_display_rect = this.DisplayRectangle;
        //    }

        //    var e = new PaintEventArgs (framebufferImageInfo.WithSize (CanvasSize), surface.Canvas, mod_scaling);
        //    OnPaintBackground (e);

        //    // border paint
        //    e.Canvas.DrawBorder (new System.Drawing.Rectangle (0, 0, (int)(scaled_client_size.Width * mod_scaling), (int)(scaled_client_size.Height * mod_scaling)), CurrentStyle);
        //    OnPaint (e);
        ...
        //    //e.Canvas.DrawRectangle (0, 0, scaled_client_size.Width - 1, scaled_client_size.Height - 1, SKColors.Red, 6);
        //}
        ...
        private void DoPaint (Rect r)
        {
            ...
            var userVisibleSize = SkiaExtensions.IgnorePixelScaling
                ? this.Size
                : ScaledSize;
            var CanvasSize = new SKSizeI ((int)(userVisibleSize.Width), (int)(userVisibleSize.Height));

            scaled_client_size = ScaledClientSize;
            scaled_display_rect = ScaledDisplayRectangle;
            ...
            var e = new PaintEventArgs (framebufferImageInfo.WithSize (CanvasSize), surface.Canvas, Scaling);
            OnPaintBackground (e);
            // border paint
