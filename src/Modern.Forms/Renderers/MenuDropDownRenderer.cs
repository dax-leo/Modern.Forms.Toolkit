...
namespace Modern.Forms.Renderers
{
    ...
    public class MenuDropDownRenderer : Renderer<MenuDropDown>
    {
        ...
        protected virtual void RenderItem (MenuDropDown control, MenuItem item, PaintEventArgs e)
        {
            ...
            //var b = item.Bounds;
            //b.Inflate (-2, -2);
            //Rectangle r = new Rectangle (item.Bounds.X, item.Bounds.Y, item.Bounds.Width - e.LogicalToDeviceUnits (1), item.Bounds.Height - e.LogicalToDeviceUnits (1));            
            e.Canvas.FillRectangle (item.Bounds, background_color);
            //if(item.Hovered) e.Canvas.DrawRectangle (b, SKColors.DarkGray); // menu item hover border style 
        ...
        protected virtual void RenderMenuSeparatorItem (MenuDropDown control, MenuSeparatorItem item, PaintEventArgs e)
        {
            ...
            e.Canvas.DrawLine (item.Bounds.X + padding.Top, center.Y, item.Bounds.Right - padding.Right, center.Y, item.Enabled ? Theme.BorderGray : Theme.DisabledTextColor, thickness);
        }
        ...
        public virtual Size GetPreferredItemSize (MenuDropDown control, MenuItem item, Size proposedSize)
        {
            ...
            return new Size (
                (int)Math.Round (text_size.Width, 0, MidpointRounding.AwayFromZero)
                + padding.Horizontal
                + control.LogicalToDeviceUnits (70),
                (int)Math.Round (text_size.Height, 0, MidpointRounding.AwayFromZero) + control.LogicalToDeviceUnits (9));
        }
